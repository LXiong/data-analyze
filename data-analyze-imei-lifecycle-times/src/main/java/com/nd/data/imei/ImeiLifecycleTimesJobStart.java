 ...
 * @author aladdin
 */
public class ImeiLifecycleTimesJobStart extends AbstractJobStart {

    public static void main(String[] args) throws Exception {
        Configuration config = HBaseConfiguration.create();
        int res = ToolRunner.run(config, new ImeiLifecycleTimesJobStart(), args);
        System.exit(res);
    }
    ...
    public Job createJob() throws Exception {
        ...
        //初始化job
        final Job job = new Job(conf, "data-analyze-imei-lifecycle-times");
        job.setJarByClass(ImeiLifecycleTimesMapred.class);
        ...
        TableMapReduceUtil.initTableMapperJob(
                inputTableName,
                scan,
                ImeiLifecycleTimesMapred.MyMapper.class,
                Text.class,
                Text.class,
                job);
        //设置环境变量
        job.getConfiguration().set(ImeiLifecycleTimesMapred.STATE_DATE_NAME, stateDate);
        job.getConfiguration().set(ImeiLifecycleTimesMapred.TABLE_NAME, inputTableName);
        //设置hdfs输出
        job.setReducerClass(ImeiLifecycleTimesMapred.MyReducer.class);
