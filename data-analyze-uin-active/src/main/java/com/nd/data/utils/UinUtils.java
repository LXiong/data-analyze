...
/**
 * Created with IntelliJ IDEA. User: Lin QiLi Date: 14-2-20 Time: 下午7:19
 */
public class UinUtils {
     ...
     * md5(imei)
     *
     * @param uin string imei
     * @param product string productId
     * @return string rowKey
     * @throws java.security.NoSuchAlgorithmException
     */
    public static String generateRowKey(String uin, String product) throws NoSuchAlgorithmException {
        MessageDigest messageDigest = MessageDigest.getInstance("MD5");
        messageDigest.update(uin.getBytes());
        byte[] digest = messageDigest.digest();
        String md5 = byteArrayToHex(digest);

        md5 = md5.substring(0, 4);

        return md5 + "_" + product + "_" + uin;
    }
